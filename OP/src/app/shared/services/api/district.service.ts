import { HttpClient, HttpParams } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { environment } from 'src/environments/environment';
import { SortUtil } from '../../infrastructure/sort.util';
import { District } from '../../models/entity/district.model';
import { ResponseModel } from '../../models/entity/response.model';
import { SelectModel } from '../../models/entity/Selected.model';
import { IdViewModel } from '../../models/viewModel/id.viewModel';
import { MsgService } from '../local/msg.service';
import { GeneralService } from './general.service';

@Injectable({
  providedIn: 'root'
})

export class DistrictService extends GeneralService {
  constructor(
    protected messageService: MsgService,
    protected httpClient: HttpClient,
  ) {
    super(messageService, httpClient, environment.opapiUrl, 'District');
  }
  //
  async getDistricts(viewModel: any): Promise<ResponseModel> {
    const param = new HttpParams();
    const res = await this.postCustomApi('getDistricts', viewModel);
    if (!this.isValidResponse(res)) { return; }
    return res;
  }

  async getDistrictsSelectModelAsync(viewModel: any): Promise<SelectModel[]> {
    const res = await this.getDistricts(viewModel);
    if (res.isSuccess) {
      const selectModel = [{ label: '-- Chọn dữ liệu --', data: null, value: null }];
      const datas = res.data as any[];
      datas.forEach(element => {
        selectModel.push({
          label: `${element.code} - ${element.name}`,
          data: element,
          value: element.id
        });
      });
      return selectModel;
    }
  }

  public getDistrictByProvinceIds(provinceIds: number[], arrCols: string[] = []): Promise<ResponseModel> {
    const obj = new IdViewModel();
    obj.ids = provinceIds;
    obj.cols = arrCols.join(',');

    return super.postCustomApi('getDistrictByProvinceIds', obj);
  }

  async getDistrictByProvinceIdsAsync(provinceIds: number[], arrCols: string[] = []): Promise<District[]> {
    const res = await this.getDistrictByProvinceIds(provinceIds, arrCols);
    if (res.isSuccess) {
      let data = res.data as District[];
      data = SortUtil.sortAlphanumericalPram(data, 'name');
      return data;
    }
    return null;
  }

  public updateDistrict(districtId: any): Promise<ResponseModel> {
    let params = new HttpParams();
    params = params.append('districtId', districtId);
    return super.getCustomApi('UpdateDistrict', params);
  }

  getDistrictByName(name: string, provinceId: number = null) {
    let params = new HttpParams();
    params = params.append('name', name);

    if (provinceId) {
      params = params.append('provinceId', provinceId.toString());
    }

    return super.getCustomApi('getDistrictByName', params);
  }

  async getDistrictAsync(viewModel: any): Promise<SelectModel[]> {
    const res = await this.getDistricts(viewModel);
    if (res.isSuccess) {
      const selectModel = [];
      let datas = res.data as any[];
      datas = SortUtil.sortAlphanumerical(datas, 1, 'code');
      datas.forEach(element => {
        selectModel.push({
          label: `${element.code} - ${element.name}`,
          data: element,
          value: element.id
        });
      });
      selectModel.unshift({ label: '-- Chọn dữ liệu --', data: null, value: null });
      return selectModel;
    }
  }
}
