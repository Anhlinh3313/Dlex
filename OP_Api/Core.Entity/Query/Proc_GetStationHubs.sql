...
SET ANSI_NULLS ON
GO
ALTER PROC Proc_GetStationHubs
(
	@PageNumber INT = null,
	@PageSize INT = NULL,
...
WITH _CTE AS(
				...
				WHEN p.IsEnabled = 0 THEN NULL
				END) AS ProvinceName, h.HasAirPort, hub.Name AS PoHubName,
				h.CenterHubId, hubCenter.Name AS hubCenterName, h.RadiusServe, h.ConcurrencyStamp,
				(ROW_NUMBER() OVER (ORDER BY hub.Id DESC)) AS RowNum
				...
				INNER JOIN dbo.Core_Hub AS hub ON hub.Id = h.POHubId AND hub.IsEnabled = 1
				INNER JOIN dbo.Core_Hub AS hubCenter ON hubCenter.id = h.CenterHubId AND hubCenter.IsEnabled = 1
				WHERE h.IsEnabled = 1 AND  h.POHubId IS NOT NULL
				AND (@CenterHubId IS NULL OR h.CenterHubId = @CenterHubId)
				AND (@POHubId IS NULL OR h.POHubId = @POHubId)
				AND (@SearchText IS NULL OR h.Code LIKE N'%' + @SearchText + '%' 
										 OR h.Name LIKE N'%' + @SearchText + '%' 
