...
{
    public class ExportExcelParternBETA
    {
        public static dynamic ExportExcel(CustomExportFile filterViewModel, DataTable dtt)
        {
            ...
            if (filterViewModel.ColumnExcelModels.Count() > 0)
            {
                ...
                for (int iR = 0; iR < dtt.Rows.Count; iR++)
                {
                    for (int iC = 0; iC < CountColumn; iC++)
                    {
                        var itemColumnModel = filterViewModel.ColumnExcelModels[iC];
                        var newRow = NewDtt.NewRow();
                        //newRow[]
                    }
                    //
                    NewDtt.Rows.Add();
                            ...
                            if (!Util.IsNull(item.TypeFormat))
                            {
                                if (item.TypeFormat.ToLower() == "number")
                                {
                                    ...
                                    for (int j = 0; j < dtt.Rows.Count; j++)
                                    {
                                        ...
                                        {
                                            var a = dtt.Rows[j][i];
                                            NewDtt.Rows[j][CountIndexColumn] = double.Parse(dtt.Rows[j][i].ToString()); //string.Format("{0:#,0}", double.Parse(dtt.Rows[j][i].ToString()));
                                        }
                                        catch (Exception ex)
                                        {
                                    ...
                                    if (item.IsSum)
                                    {
                                        NewDtt.Rows[dtt.Rows.Count][indexSum] = sum[indexSum]; //string.Format("{0:#,0}",sum[indexSum]);
                                    }
                                }
